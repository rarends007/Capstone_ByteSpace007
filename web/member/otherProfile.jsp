<%-- Document : index Created on : Mar 18, 2026, 5:27:08 AM Author : raren --%>
                    ...
                    <section>
                        <c:forEach var="post" items="${posts}">
                            <div class="post_container">
                                <div class="post_header">
                                    <c:choose>
                                        <c:when test="${loaded_profile_profile_photo != null}">
                                            <img src="${loaded_profile_profile_photo}" alt="Profile Image"
                                        class="profile_image make_post_img">
                                        </c:when>
                                        <c:otherwise>
                                ...
                                <p class="post_text">
                                    <c:out value="${post.value.postText}" />
                                </p>
                                <c:forEach var="image" items="${post.value.images}">
                                    <img src="${image.value.imagePath}" class="post_img" />
                                </c:forEach>
                                <button id="commentsBtn-${post.value.postID}" class="ui_btn commentsBtn"> <img
                                        src="${pageContext.request.contextPath}/img/comment.svg" />${post.value.comments.size()}
                                    Comments </button>

                                <div class="comments_container hidden" id="commentsContainer-${post.value.postID}">
                                    <button id="close_comments"><img
                                            src="${pageContext.request.contextPath}/img/close.svg" /></button>
                                    <c:forEach var="comment" items="${post.value.comments}">
                                        <div class="comment_container">
                                            <div class="comment_header">
                                                <h4 class="comment_heading">${comment.value.commentingUsername}</h4>
                                                <c:if
                                                    test="${loadedProfileUsername.equals(comment.value.commentingUsername)}">
                                                    <form action="${pageContext.request.contextPath}/Member"
                                                        method="POST">
                                                        <input type="hidden" name="action" value="delete_comment" />
                                                        <input type="hidden" name="comment_id"
                                                            value="${comment.value.commentID}" />
                                                        <input type="hidden" name="to_other" value="true" />

                                                        <button type="submit" value="Delete" class="delete_comment_btn">
                                                            <img
                                                                src="${pageContext.request.contextPath}/img/delete.svg" />
                                                        </button>
                                                        <!-- delete a comment on click -->
                                                    </form>
                                                </c:if>
                                                <!--Protects comments the user did not make from the user deleting them on the viewed profile. -->
                                            </div>
